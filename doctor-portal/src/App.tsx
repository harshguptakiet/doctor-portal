...
const App: React.FC = () => {
  ...
  const QueueView = () => {
    ...
    const handleCallNext = async () => {
      ...
      if (nextPatient) {
        ...
        if (patientPhone) {
          ...
          if (activeCall) {
            const switchCall = window.confirm(`You have an active call with ${activeCall.patientName}. End current call and start calling next patient ${patientName}?`);
            if (!switchCall) return;
          }

          const confirmed = window.confirm(`Call next patient: ${patientName} at ${patientPhone}?`);
          if (confirmed) {
            // Start the call using unified system
            setActiveCall({
            ...
            // Don't change status here - keep it as 'scheduled' until call ends
            // The status will be updated to 'completed' when the call ends
          }
        } else {
          // If no phone number, ask if they want to mark as completed
          const confirmed = window.confirm(`No phone number for ${patientName}. Mark appointment as completed?`);
          if (confirmed) {
            try {
              await updateAppointmentInFirebase(nextPatient.id, { status: 'completed' });
            } catch (error) {
    ...
    const handleCallPatient = (patientPhone: string, patientName: string, appointmentId?: string) => {
      if (patientPhone) {
        // Check if there's already an active call
        if (activeCall) {
          const switchCall = window.confirm(`You have an active call with ${activeCall.patientName}. End current call and start new call with ${patientName}?`);
          if (!switchCall) return;
        }

        const confirmed = window.confirm(`Start call with ${patientName} at ${patientPhone}?`);
        if (confirmed) {
